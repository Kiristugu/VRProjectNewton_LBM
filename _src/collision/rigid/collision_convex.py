...
from .multicontact import create_build_manifold, create_build_manifold_values, empty_contact_values, set_contact_value
...
def create_solve_convex_multi_contact_values(support_func: Any, post_process_contact: Any):
    """Create a GJK/MPR multi-contact solver that returns fixed-size contact values."""

    @wp.func
    def solve_convex_multi_contact_values(
        geom_a: Any,
        geom_b: Any,
        orientation_a: wp.quat,
        orientation_b: wp.quat,
        ...
        if skip_multi_contact or signed_distance > contact_threshold:
            ...
            distances, penetrations, positions, normals, tangents = set_contact_value(
                0, contact_data, distances, penetrations, positions, normals, tangents
            )

            return 1, distances, penetrations, positions, normals, tangents

        return wp.static(create_build_manifold_values(support_func, post_process_contact))(
            geom_a,
            geom_b,
            orientation_a,
            orientation_b,
    ...
    return solve_convex_multi_contact_values
