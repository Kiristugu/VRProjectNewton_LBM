...
@wp.func
def empty_contact_values() -> tuple[_vec5, _vec5, _mat53f, _mat53f, _mat53f]:
    distances = _vec5()
    penetrations = _vec5()
    positions = _mat53f()
    normals = _mat53f()
    ...
    for i in range(5):
        distances[i] = MAXVAL
        penetrations[i] = MAXVAL
        positions[i] = wp.vec3()
        normals[i] = wp.vec3()
        tangents[i] = wp.vec3()

    return distances, penetrations, positions, normals, tangents


@wp.func
def contact_data_values(contact_data: ContactData) -> tuple[float, float, wp.vec3, wp.vec3, wp.vec3]:
    normal = wp.normalize(contact_data.contact_normal_a_to_b)
    a_contact_world = contact_data.contact_point_center - normal * (
        0.5 * contact_data.contact_distance + contact_data.radius_eff_a
    )
    b_contact_world = contact_data.contact_point_center + normal * (
        0.5 * contact_data.contact_distance + contact_data.radius_eff_b
    )
    distance = wp.dot(b_contact_world - a_contact_world, normal)
    total_separation_needed = (
        contact_data.radius_eff_a + contact_data.radius_eff_b + contact_data.margin_a + contact_data.margin_b
    )
    penetration = distance - total_separation_needed

    world_x = wp.vec3(1.0, 0.0, 0.0)
    if wp.abs(wp.dot(normal, world_x)) > 0.99:
        world_x = wp.vec3(0.0, 1.0, 0.0)
    tangent = wp.normalize(world_x - wp.dot(world_x, normal) * normal)

    return contact_data.contact_distance, penetration, contact_data.contact_point_center, normal, tangent
...
def set_contact_value(
    ...
    normals: _mat53f,
    tangents: _mat53f,
) -> tuple[_vec5, _vec5, _mat53f, _mat53f, _mat53f]:
    if slot < 5:
        distance, penetration, position, normal, tangent = contact_data_values(contact_data)
        distances[slot] = distance
        penetrations[slot] = penetration
        positions[slot, 0] = position[0]
        positions[slot, 1] = position[1]
        positions[slot, 2] = position[2]
        normals[slot, 0] = normal[0]
        normals[slot, 1] = normal[1]
        normals[slot, 2] = normal[2]
        tangents[slot, 0] = tangent[0]
        tangents[slot, 1] = tangent[1]
        tangents[slot, 2] = tangent[2]
...
def create_build_manifold_values(support_func: Any, post_process_contact: Any):
    """Create a manifold builder that returns fixed-size contact value arrays."""

    @wp.func
    def extract_4_point_contact_manifolds_values(
        m_a: wp.array(dtype=wp.vec2),
        m_a_count: int,
        m_b: wp.array(dtype=wp.vec2),
        m_b_count: int,
        ...
        normals: _mat53f,
        tangents: _mat53f,
    ) -> tuple[int, float, _vec5, _vec5, _mat53f, _mat53f, _mat53f]:
        normal_dot = wp.abs(wp.dot(projector_a.normal, projector_b.normal))
        ...
        if loop_count > 1:
            ...
            for i in range(loop_count):
                ...
                distances, penetrations, positions, normals, tangents = set_contact_value(
                    i, contact_data, distances, penetrations, positions, normals, tangents
                )
        else:
            normal_dot = 0.0
            loop_count = 0

        return loop_count, normal_dot, distances, penetrations, positions, normals, tangents

    @wp.func
    def build_manifold_values(
        geom_a: Any,
        geom_b: Any,
        quaternion_a: wp.quat,
        quaternion_b: wp.quat,
        ...
        distances, penetrations, positions, normals, tangents = empty_contact_values()
        ...
        if a_count < 2 or b_count < 2:
            ...
            if excess_normal_deviation(normal, projector_a.normal) or excess_normal_deviation(
                ...
                (
                    ...
                    normals,
                    tangents,
                ) = extract_4_point_contact_manifolds_values(
                    a_buffer,
                    a_count,
                    b_buffer,
                    b_count,
        ...
        if should_include_deepest_contact(normal_dot) or count_out == 0:
            ...
            distances, penetrations, positions, normals, tangents = set_contact_value(
                count_out, contact_data, distances, penetrations, positions, normals, tangents
            )
            count_out += 1

        return count_out, distances, penetrations, positions, normals, tangents

    return build_manifold_values
