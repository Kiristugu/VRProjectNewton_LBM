...
from newton._src.geometry.support_function import extract_shape_data
...
def _clear_active_kernel(
    ...
    """Kernel to clear active hashtable entries and their value slots.
    ...
    while i < total_work:
        ...
        # Clear keys only once per entry (when processing slot 0)
        if local_idx == 0:
...
class GlobalContactReducer:
    ...
    **Contact Data Storage:**
    ...
    def __init__(
        ...
        # - Assume ~8 contacts per unique key on average for dense mesh contacts
    ...
    def clear_active(self):
        ...
        Uses a combined kernel that clears hashtable keys and values, followed by a
        small kernel to zero the counters.
        """
        # Use fixed thread count for efficient GPU utilization
        num_threads = min(1024, self.hashtable.capacity)

        # Single kernel clears keys and values for active entries
        wp.launch(
