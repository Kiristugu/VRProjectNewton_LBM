...
from wanphys.fluid import ParticleFluidModel, ParticleFluidDomain
...
class SensorTiledCamera:
    ...
    # TODO: implement multi-models for wanphys framework

    def __init__(self, domain: RigidDomain, particle_domain: ParticleFluidDomain = None, **kwargs):
        self.domain = domain
        ...
        self.render_context = RenderContext(
            ...
            config=self.RenderContext.Config(
        ...
        # TODO: add soft model and fluid model
        if particle_domain is not None:
            if particle_domain.state.particle_q is not None and particle_domain.state.particle_q.shape[0]:
                self.render_context.particles_position = particle_domain.state.particle_q
                self.render_context.particles_radius = particle_domain.model.particle_radius
                self.render_context.particles_world_index = particle_domain.model.particle_world_ids
    ...
    def sync_transforms(self, state: DomainState):
    ...
    def compute_pinhole_camera_rays(
        ...
        width: int,
        height: int,
        camera_fovs: float | list[float] | np.ndarray | wp.array(dtype=wp.float32),
    ...
    def flatten_color_image_to_rgba(
        ...
        out_buffer: wp.array(dtype=wp.uint8, ndim=4) | None = None,
