...
def compute_sensor_imu_kernel(
    ...
    # TODO: Refactor. Currently adapted from Newton source.
...
class SensorIMU:
    ...
    Example:
        Create a SensorIMU for a rigid domain with site labels::

            from wanphys.rigid import RigidDomain, RigidModelBuilder
            from wanphys.sensors import SensorIMU

            builder = RigidModelBuilder()
            # Add bodies and labeled sensor sites here.
            model = builder.finalize()
            domain = RigidDomain(model)
            domain.create_state()

            sensor = SensorIMU(domain, sites="imu_*")
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            domain: The rigid domain to use.
        ...
        site_indices: list[int] = match_labels(model.shape_label, sites)
        if not site_indices:
        ...
        self._validate_sensor_sites(site_indices)

        self.sensor_sites_arr: wp.array = wp.array(site_indices, dtype=int, device=model.device)
        self.n_sensors: int = len(site_indices)
        ...
        if self.verbose:
            ...
            print(f"  Sites: {len(set(site_indices))}")
            # TODO: body per site
    ...
    def _validate_sensor_sites(self, sensor_sites: list[int]):
        ...
        for site_idx in sensor_sites:
            ...
            if not (shape_flags[site_idx] & NewtonShapeFlags.SITE):
