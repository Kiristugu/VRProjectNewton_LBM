...
@wp.func
def tid_to_coord_pixel_priority(
    tid: wp.int32,
...
@wp.func
def tid_to_coord_view_priority(
    tid: wp.int32,
    camera_count: wp.int32,
    width: wp.int32,
    height: wp.int32,
...
def render_megakernel(
    ...
    if render_order == RenderOrder.PIXEL_PRIORITY:
        ...
        world_index, camera_index, py, px = tid_to_coord_tiled(tid, camera_count, img_width, img_height, tile_width, tile_height)
    else :
        return 
    ...
    color = wp.vec4f(1.0)
    si = closest_hit.shape_index
    if si < raytrace.MAX_SHAPE_ID:
        color = shape_colors[si]
        if shape_materials[si] > -1:
            color = wp.cw_mul(color, material_rgba[shape_materials[si]])

    base_color = wp.vec3f(color[0], color[1], color[2])
    out_color = wp.vec3f(0.0)

    if enable_textures and si < raytrace.MAX_SHAPE_ID:
        material_index = shape_materials[si]
        if material_index > -1:
