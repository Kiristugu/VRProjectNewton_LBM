...
########################################################################################################################
######################################    Test Solver For CollisionTriMeshVBD    #######################################
########################################################################################################################
...
from wanphys._src.geometry.collision_trimesh_vbd import (
    CollisionTriMeshVBD,
    CollisionTriMeshVBDInfo,
    apply_planar_truncation_parallel_by_collision_vbd,
    accumulate_contact_force_and_hessian_from_geometry,
)

# Export accumulate_contact_force_and_hessian for legacy collision_legacy.py compatibility
__all__ = ["SolverVBDTest"]


class SolverVBDTest(SolverBase):
    ...
    def _init_particle_system(
        ...
        if particle_enable_self_contact:
            ...
            self.trimesh_collision_detector = CollisionTriMeshVBD(
                self.model,
            ...
            self.trimesh_collision_info = wp.array(
                [self.trimesh_collision_detector.collision_info], dtype=CollisionTriMeshVBDInfo, device=self.device
            )
    ...
    def _penetration_free_truncation(self, particle_q_out=None):
        ...
        if not self.particle_enable_self_contact:
            ...
            self.truncation_ts.fill_(1.0)
            wp.launch(
                kernel=apply_planar_truncation_parallel_by_collision_vbd,
                inputs=[
    ...
    def _solve_particle_iteration(
        ...
        for color in range(len(self.model.particle_color_groups)):
            if contacts is not None and not self.particle_enable_self_contact:
                wp.launch(
            ...
            if self.particle_enable_self_contact:
                wp.launch(
                    kernel=accumulate_contact_force_and_hessian_from_geometry,
                    dim=self.particle_self_contact_evaluation_kernel_launch_size,
                    inputs=[
                        dt,
                        color,
                        ...
                        contacts.soft_contact_particle if contacts is not None else wp.zeros(0, dtype=int, device=self.device),
                        contacts.soft_contact_count if contacts is not None else wp.zeros(1, dtype=int, device=self.device),
                        contacts.soft_contact_max if contacts is not None else 0,
                        self.body_particle_contact_penalty_k if contacts is not None else wp.zeros(0, dtype=float, device=self.device),
                        self.body_particle_contact_material_kd if contacts is not None else wp.zeros(0, dtype=float, device=self.device),
                        self.body_particle_contact_material_mu if contacts is not None else wp.zeros(0, dtype=float, device=self.device),
                        ...
                        body_qd_for_particles,
                        self.model.body_com,
                        contacts.soft_contact_shape if contacts is not None else wp.zeros(0, dtype=int, device=self.device),
                        contacts.soft_contact_body_pos if contacts is not None else wp.zeros(0, dtype=wp.vec3, device=self.device),
                        contacts.soft_contact_body_vel if contacts is not None else wp.zeros(0, dtype=wp.vec3, device=self.device),
                        contacts.soft_contact_normal if contacts is not None else wp.zeros(0, dtype=wp.vec3, device=self.device),
                    ],
                    outputs=[self.particle_forces, self.particle_hessians],
                    device=self.device,
    ...
    def _collision_detection_penetration_free(self, current_state: State):
        ...
        self.trimesh_collision_detector.collision_detection_penetration_free(
            current_state,
            self.particle_conservative_bound_relaxation,
            self.particle_self_contact_margin,
            particle_rest_shape_contact_exclusion_radius=self.particle_rest_shape_contact_exclusion_radius,
            particle_q_rest=self.particle_q_rest,
        )
        self.particle_conservative_bounds.assign(self.trimesh_collision_detector.particle_conservative_bounds)
        self.trimesh_collision_detector.build_contact_geometry(current_state.particle_q)
