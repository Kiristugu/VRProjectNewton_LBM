...
from wanphys._src.geometry.tests.collision_trimesh_style3d.style3d_solver_test import SolverStyle3DTest
...
class Example:
    def __init__(self, viewer, args=None):
        ...
        self.garment_mesh_points = garment_mesh.vertices[:, [2, 0, 1]]
        ...
        self.body_q_0 = wp.clone(self.state.body_q, device=self.model.device)
        self.body_q_1 = wp.clone(self.state.body_q, device=self.model.device)
        ...
        self.cloth_solver = SolverStyle3DTest(
            model=self.model,
    ...
    def _force_update_targets(self):
        key_time = [2.0, 6.0, 10.0]
        target_pos = [
            [wp.vec3(0.16, 0.65, 1.71), wp.vec3(0.28, -0.50, 1.19)],
            [wp.vec3(0.12, 0.34, 0.99), wp.vec3(0.14, -0.35, 0.97)],
        ]
        target_rot = [
            [wp.quat(0.58, -0.35, 0.29, 0.68), wp.quat(0.00, 0.00, 0.00, 0.00)],
            [wp.quat(-0.09, 0.46, 0.03, 0.88), wp.quat(-0.09, 0.48, 0.01, 0.87)],
        ]
