...
def plot_cavity_field_diff(
    ...
    *,
    ...
    """Plot absolute BGK-MRT difference on the cavity mid x-y plane."""
    plt = _require_matplotlib()

    if diff.ndim != 3:
        raise ValueError(f"diff must have shape (nx, ny, nz), got {diff.shape}")

    nx: int = int(diff.shape[0])
    ny: int = int(diff.shape[1])
    nz: int = int(diff.shape[2])
    k: int = (nz // 2) if slice_k is None else int(slice_k)
    if k < 0 or k >= nz:
        raise ValueError(f"slice_k={k} out of range for nz={nz}")

    diff_2d: np.ndarray = diff[:, :, k].T
    domain_x: float = nx * cell_size
    domain_y: float = ny * cell_size
    x: np.ndarray = np.linspace(0.0, domain_x, nx)
    y: np.ndarray = np.linspace(0.0, domain_y, ny)
    ...
    fig, ax = plt.subplots(figsize=figsize)
    contour = ax.contourf(x_mesh, y_mesh, diff_2d, levels=n_contour_levels, cmap=cmap)
    cbar = fig.colorbar(contour, ax=ax, fraction=0.046, pad=0.04)
    cbar.set_label(f"|BGK-MRT| {field}")
    ...
    if path is not None:
        ...
        out.parent.mkdir(parents=True, exist_ok=True)
        fig.savefig(out, dpi=dpi)
        print(f"Saved cavity diff plot: {out}")
